...
class BannersController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->modal = 'App\Models\Banner';
        $this->route = new \stdClass;
        $this->module = 'Banner';
        $this->route->list = route('admin.banners.list');
        $this->route->add = route('banners.create');
        $this->route->store = route('banners.store');
        $this->route->edit = route("banners.edit", ":id");
        $this->route->destroy = route("banners.destroy", ":id");
        $this->route->upload = route("banners.upload");
    }
    ...
    public function list(Request $request)
    {
        ...
        foreach ($result as $row) {
            ...
            if ($row->status == 1) {
                $status = '<div class="form-check form-switch">  <input class="form-check-input" type="checkbox" id="active_check" checked="" onchange=status_change("' . $status_url . '",this,' . $row->id . ');>   </div>';
            } else {
                $status = '<div class="form-check form-switch"> <input class="form-check-input" type="checkbox" id="unactive_check" onchange=status_change("' . $status_url . '",this,' . $row->id . ');> </div>';
            }
