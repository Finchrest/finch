...
class CustomPassportOrdersController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->route->list = route('superadmin.custom_passport_orders.list');
        $this->route->add = route('custom_passport_ordersData.create');
        $this->route->store = route('custom_passport_ordersData.store');
        $this->route->show = route("custom_passport_ordersData.show", ":id");
    }
    ...
    public function create()
    {
        ...
        $allcoupans = Coupon_redeemed::get();
        return view(
    ...
    public function store(Request $request)
    {
        ...
        if ($id) {
            ...
            if ($validator->fails()) {
                ...
                $obj->coupon_redeem_ids = $coupons;
                $obj->is_custom = 1;
                $obj->save();

                $pid = $obj->id;

                // $getOrder = PassportOrder::where(['id'=>$id])->first();
                //    echo '<pre>pay_data - '; print_r($getOrder); die;
