    ...
    { 
        ...
        if($id){
            ...
            if ($validator->fails()) {
                ...
                $name2 = Str::slug($request->name,'-');   
                $slug = $name2.'-'.rand(10,99).date('s');
                ...
                $obj->save();
                echo "<pre>";print_R($obj->toArray());die;

                $newSlug = $name2.'-'.@$obj->id.rand(10,99);
                $this->modal->where(['id'=>@$obj->id])->update(['slug'=>$newSlug]);
