...
class PassportOrdersController extends Controller
{
    ...
    public function getPassportFreeProduct(Request $request)
    {
        ...
        if ($user = JWTAuth::parseToken()->authenticate()) {
            ...
            $passportOderData = PassportOrder::where(['passport_code' => $request->passport_code, 'user_id' => $user->id])->first();
            if (!empty($passportOderData)) {
                $passport_id = $passportOderData->passport_id;

                $product_arr = array();
                $freePassportId = explode(',', $passportOderData->is_free);
                $freeProducts = PassportFreeItem::select('products.*', 'passport_free_item.id as freeId', 'passport_free_item.passport_id', 'passport_free_item.status as freestatus', 'passport_free_item.created_at as freeCreated_at', 'passport_free_item.updated_at as freeUpdated_at')->where('passport_id', $passport_id)->where('products.status', 1)->whereNotIn('products.id', $freePassportId)->join('products', 'products.id', '=', 'passport_free_item.product_id')->get();

                $i = 0;
                foreach ($freeProducts as $products) {
                    // $product_arr[$i] = $freeProduct;
                    $product_arr[$i]['id'] = $products->freeId;
                    $product_arr[$i]['passport_id'] = $products->passport_id;
                    $product_arr[$i]['product_id'] = $products->id;
                    $product_arr[$i]['status'] = $products->freestatus;
                    $product_arr[$i]['created_at'] = $products->freeCreated_at;
                    $product_arr[$i]['updated_at'] = $products->freeUpdated_at;
                    $product_arr[$i]['title'] = strtoupper(@$products->title);
                    $product_arr[$i]['image'] = asset(@$products->FileId->file);
                    $product_arr[$i]['sub_title'] = ucwords(strtolower($products->sub_title));
                    $product_arr[$i]['price'] = 0;
                    $product_arr[$i]['tax'] = 0;
                    $product_arr[$i]['total_price'] = 0;
                    $i++;
                }

                return response()->json([
                    'success' => true,
                    'message' => 'Passport free product list',
                    'data' => $product_arr
                ], Response::HTTP_OK);
            } else {
                return response()->json([
