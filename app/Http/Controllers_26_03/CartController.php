...
class CartController extends Controller
{
    ...
    public function addToCart(Request $request){  
		...
		if(isset((auth()->user()->id)) && !empty(auth()->user()->id)){
			...
			// echo '<pre>product_arr - '; print_r($product_arrs); 
			// echo '<pre>product - '; print_r($product); die;
			// echo '<pre>product - '; print_r($product->toArray()); die;
			...
			if($product->type == 1 && $age < 21){
				...
				if(empty($placeArr) || in_array($product['place'],$placeArr)){
					...
					$adata =array(
					 ...
					 //echo '<pre>adata - ';print_R($adata);die;
					Cart::add($adata);
					$product_arr = $this->getCartProduct();
					...
					return response()->json(['success'=>1,'cart_view'=>$cart_view,'pid'=>$pid,'product_view'=>$product_view,'cart_count'=>Cart::count(),'cart_total'=>Cart::subtotal(),'message'=>'Product added in cart']);
				}else{
    ...
    { 
	  ...
	  */
		...
		// echo '<pre>';print_r($product_arr);die;
		return view('cart.checkout',['page' =>$this->page,'product_arr'=>$product_arr,'total'=>$total]);
    }
    ...
    {
		//echo '<pre>request - ';print_r($request->all());die;
		// return response()->json(['success'=>1,'message'=>"Payment Process, Please Wait..,",'id'=>56]);
		// die;
        ...
        if ($validator->fails()) {
			...
			if(Cart::count() > 0){
				...
				// echo '<pre>product_arrs - ';print_r($product_arrs);die;
				...
				$passport_code = $coupon_code = ''; $passport_pay = $coupon_percent = $coupon_amount = 0;
    ...
    public function cart_checkout_view(Request $request){
		...
		$view = view('cart/checkout_view',['amt'=>$request->amt,'codeType'=>$request->codeType,'codeNum'=>$request->codeNum,'codeAmt'=>$request->codeAmt]);
