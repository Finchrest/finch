...
class PassportOrdersController extends Controller
{
    ...
    public function show($id)
    {
        ...
        foreach ($items as $item) {
            if (strpos($item->order_date, "-")) {
                $formateTime = date('d/m/Y', strtotime($item->order_date));
            } else {
                $formateTime = date('d/m/Y', $item->order_date);
            }

            $order_data['order']['items'][$i]['order_date'] =  $formateTime;
    ...
    public function search(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if (empty($info)) {
                ...
                $info_data = $this->modal::find($info['id']);

                $items = array();
                $order_data = array();
                $passportOrder = PassportOrder::select('id', 'passport_code', 'passport_id', 'name', 'phone', 'email', 'price', 'payment_id', 'order_date', 'payment_date', 'volume_amount', 'used_amount', 'remaining_amount')->where('id', $info_data['id'])->first()->toArray();
                $order_data['order'] = $passportOrder;
                $passport_code = $passportOrder['passport_code'];
                ...
                $view = view('admin.user.user_search', ['info' => $info, 'info_data' => $order_data, 'route' => $this->route, 'module' => $this->module]);
                return response()->json(['success' => 1, 'view' => $view->render()]);
            }
        }
    }
