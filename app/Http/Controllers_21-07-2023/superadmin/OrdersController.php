...
class OrdersController extends Controller
{
  ...
  public function __construct()
  {
    ...
    $this->module = 'Order';
    $this->route->list = route('superadmin.orders.list');
    $this->route->add = route('ordersData.create');
    $this->route->show = route("ordersData.show", ":id");
    $this->route->orderStatus = route("superadmin.ordersData.orderStatus", ":id");
    $this->route->changeStatus = route("superadmin.ordersData.changeStatus");
    $this->route->changeItemStatus = route("superadmin.ordersData.changeItemStatus");
    $this->route->refund_orders = route("superadmin.refund_orders");
    $this->route->refund_orders_list = route("superadmin.refund_orders_list");
    ...
    if ($searchValue) {
      $qry->where(function ($query) use ($searchValue) {
        $query->where('orders.name', 'like', '%' . $searchValue . '%')
          ->orWhere('places.name', 'like', '%' . $searchValue . '%')
  ...
  public function changeStatus(Request $request)
  {
    ...
    if ($id) {
      ...
      if ($validator->fails()) {
        ...
        $order_total = $order_total_actual * 100;
        $user_id = $obj->user_id;
