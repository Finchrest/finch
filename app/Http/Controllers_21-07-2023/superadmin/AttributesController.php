...
class AttributesController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->route->list = route('superadmin.attributes.list');
        $this->route->add = route('attributesData.create');
        $this->route->store = route('attributesData.store');
        $this->route->edit = route("attributesData.edit", ":id");
        $this->route->destroy = route("attributesData.destroy", ":id");
    }
        ...
        if ($searchValue) {
            $qry->where(function ($query) use ($searchValue) {
                $query->where('attributes.name', 'like', '%' . $searchValue . '%')
                    ->orWhere('attribute_options.option_name', 'like', '%' . $searchValue . '%');
            });
        }
