...
<div class="modal-body p-md-5 p-4">
  <div class="FoodDetailsHere PassportDetailsHere UserDetails">
    <div class="PassportDetails3 col-md-10 m-auto p-0">
      <h4 class="text-center">Hi, {{ucwords($passportConfirmOrder->name)}}</h4>
      <div class="CansumerFormTable w-100 text-white">
        <form onsubmit="passportConfirmSubmit(this); return false;">
          <div class="TableBorderBox mt-5">
            <table class="table table-border TableTwo m-0">
              ...
              <tr>
                <th>Price:</th>
                <th><i class="fa fa-inr" aria-hidden="true"></i> {{number_format($passportConfirmOrder->price,2)}}</th>
              </tr>
              <tr>
                <th>Date:</th>
            ...
            <input type="hidden" name="payment_id" value="{{$passportConfirmOrder->payment_id}}">
            ...
            <input type="hidden" name="subtotal" value="{{$passportConfirmOrder->price}}">
...
<script>
  ...
  function passportConfirmSubmit(e){
    $.ajaxSetup({
      headers: {
    ...
    $.ajax({  
      url :"{{route('passportConfirmSubmit')}}",  
      method:"POST",  
      dataType:"json",
      data:$(e).serialize(),  
  ...
  function razorpay_payment_view(port_id){
    var options = {
      ...
      "handler": function (response){
          ...
          $.ajax({  
            url :"{{ route('passport.passport_payment_submit') }}",  
            method:"POST",  
            dataType:"json",  
  ...
  function getPassportSummaryView(id){
    $.ajaxSetup({
      headers: {
        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
      }
    ...
    $.ajax({  
      url :"{{route('getPassportSummaryView')}}",  
      method:"POST",  
      dataType:"json",  
      data:{id:id},
      success: function(data){ 
