              ...
              <div class="fbBestPlacer w-100 clearfix">
                <div class="PrimePlacesDetails w-100 text-center">
                  ...
                  <div class="GrabBtn3 pt-2">
                    <label for="location_id_{{$location->id}}" class="btn m-0 onOrder onOrder2 position-relative <?php if ($setLocation == $location->id) {
                                                                                                                    echo 'checked';
                                                                                                                  } ?>">
                      <input type="radio" name="location_id" <?php if ($setLocation == $location->id) {
                                                                echo 'checked';
                                                              } ?> value="{{$location->id}}" onchange="setLocations(this,'changeLcoationForm'); return false;" id="location_id_{{$location->id}}" class="position-absolute">
                      {{$location->name}}
                    </label>
      ...
      <tbody>
        ...
        <tr>
          <td>
            <input type="radio" name="location_id" <?php if ($setLocation == $location->id) {
                                                      echo 'checked';
                                                    } ?> value="{{$location->id}}" onchange="setLocations(this,'changeLcoationForm'); return false;"> &nbsp; &nbsp; <img src="{{$loc_image}}" class="img-thumbnail bg-dark" alt="" width="50" height="50"> &nbsp; &nbsp; {{$location->name}}
          </td>
        </tr>
        @endforeach
      </tbody>
