          ...
          <form id="general_setting" role="form" action="" method="post" onsubmit="update_data(this);return false;">
            ...
            <div class="card-body">
              <div class="row">
                ...
                <div class="col-md-12">
                  <div class="form-group ">
                    <label>Minimum Purchse Amount<sup class="text-danger">*</sup></label>
                    <input type="text" class="form-control" name="settings[minimum_purchase_amount]" value="{{ $settings['minimum_purchase_amount'] }}">
                  </div>
                </div>
                <div class="col-md-6">
                <label>Delivery Charges<sup class="text-danger">*</sup></label>
                <div class="input-group mb-3">
                 <div class="input-group-prepend">
                 <span class="input-group-addon" id="basic-addon3">₹</span>
                  </div>
                     <input type="text" class="form-control" name="settings[delivery_charges]" value="{{ $settings['delivery_charges'] }}">
                   </div>
                </div>
             ...
             <input type="text" class="form-control" placeholder="Tax"  name="settings[deliveryTax]" value="{{ $settings['deliveryTax'] }}">
             </div>
             </div>
...
<script>
  ...
  function update_data(e) {
    ...
    $.ajax({
      url: "{{ route('admin.settings.general_setting') }}",
      headers: {
  ...
  function upload_site_logo(id) {
    ...
    $.ajax({
      type: "POST",
      url: "{{ route('admin.settings.upload_site_logo') }}",
      headers: {
        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
      },
      contentType: false,
...
$.ajax({
  type: "POST",
  url: "{{ route('admin.settings.upload_bell_sound') }}",
  headers: {
    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
  },
  contentType: false,
  ...
  function update_password(e) {
    ...
    $.ajax({
      url: "{{ route('admin.settings.update_password') }}",
      headers: {
        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
      },
      method: "POST",
