...
<section class="w-100 clearfix fbOrderMenus py-5 mt-md-5" id="fbOrderMenus" style="background-image:url({{ asset('front-assets/images/banner5.png') }});">
  ...
  <div class="container-fluid py-md-5">
    <div class="bradcrump d-flex justify-content-between drinkBradcrump mx-xl-5 pb-3">
      <ul class="list-unstyled DrinkBrad">
        ...
        <?php if ($meals_count > 0) { ?>
          <li><a href="{{url('product/'.$slug_name.'/meals')}}" class="active meal_menu" onclick="showProduct(this,2,{{ $place }}); return false;">Meals ({{ $meals_count }})</a></li>
        <?php } ?>
      </ul>
      <ul class="rightList d-flex align-items-center FilterBarMeals">
        <li>
          <form>
            ...
            <dd>
              <div class="mutliSelect">
                <ul>
                  ...
                  <li>
                    <label class="mb-0" for="cat_{{$cat->id}}"><input type="checkbox" value="{{$cat->id}}" class="catID" id="cat_{{$cat->id}}" onclick="selectCat(this,{{ $place }},{{ $cat->id }})"><input type="hidden" name="cat_name" value="{{$cat->name}}" id="catName">{{$cat->name}}</label>
                  </li>
                  @endforeach
                </ul>
              </div>
  ...
  function showProduct(e, type, place) {
    ...
    $.ajax({
      ...
      data: {
        ...
        meals_count: $('#meals_count').val(),
        _totalCurrentResult: 0
      },
      success: function(data) {
        if (data.success == 1) {
    ...
    if (cat_value != '' || search_text != '') {
      ...
      $.ajax({
        ...
        data: {
          ...
          drink_count: $('#drink_count').val(),
          meals_count: $('#meals_count').val(),
          _totalCurrentResult: _totalCurrentResult
        },
        success: function(data) {
          if (data.success == 1) {
