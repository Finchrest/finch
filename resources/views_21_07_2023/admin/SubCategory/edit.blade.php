			...
			<div class="row">
                  <div class="col-md-6">
                     <div class="form-group ">
                        ...
                        <input type="text" name="name" class="form-control" placeholder="Enter Category name" value="{{ $info->name }}">
                     </div>
                  </div>
                  <div class="col-md-6">
                     <div class="form-group ">
                        <label>Categories<sup class="text-danger">*</sup></label>
                        <select name="category" class="form-control select_category">
                           <option value="">Select</option>
                           @foreach($categories as $category)
                              <option value="{{$category->id}}" {{ ($info->category_id == $category->id)? 'selected': ''}}>{{ucfirst($category->name)}}</option>
                           @endforeach
						      </select>
                     </div>
                  </div>
